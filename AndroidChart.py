...
if __name__ == '__main__':
    ...
    for item in range(1000):
        ...
        cpu_avg = cpu_sum / int(item + 1) * 100

        cpu_avg = "{}%".format(round(cpu_avg), 2)

        # mem_list += mem_info

        Native_Heap_sum = Native_Heap_sum + mem_info[0]

        Dalvik_Heap_sum = Dalvik_Heap_sum + mem_info[1]

        Native_Heap_avg = round(Native_Heap_sum / int(item + 1), 2)

        Dalvik_Heap_avg = round(Dalvik_Heap_sum / int(item + 1), 2)
